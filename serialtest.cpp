...
//========================================================fcg step motor
        else if (receivedata.mid(28,2) == "0d") {
            //stepmotor+fcg
            StepMotorAngle = (receivedata.mid(4,4).toInt(&ok, 16));
//            if(mag_user1 >= 32768){
//                mag_user1 -= 65536;
//            }
            StepMotorSpeed = (receivedata.mid(8,2).toInt(&ok, 16));
            //=============fcg for StepMotor
            addserialSaveAndApp("StepMotorSpeed: " +
                                    getStepmotor_Speed() + " " );
        }
        //========================================================fcg BDC motor
                else if (receivedata.mid(28,2) == "0e") {
                    //stepmotor+fcg
                    BDCMotorAngle = (receivedata.mid(4,4).toInt(&ok, 16));
        //            if(mag_user1 >= 32768){
        //                mag_user1 -= 65536;
        //            }
                    BDCMotorSpeed = (receivedata.mid(8,4).toInt(&ok, 16));
                    //=============fcg for BDCMotor
                    addserialSaveAndApp("BDCMotorSpeed: " +
                                            getBDCmotor_Angle() + " " );
                }
//==============================================================ace for ESC
        else if (receivedata.mid(28,2) == "0a") {
            //stepmotor+fcg
            ReadESC_Speed = (receivedata.mid(4,4).toInt(&ok, 16));
//            if(mag_user1 >= 32768){
//                mag_user1 -= 65536;
//            }
            ReadESC_Thro = (receivedata.mid(10,2).toInt(&ok, 16));
//=============ace for esc
            addserialSaveAndApp("ESC_Data: " +
                                    getESC_Speed() + " " +
                                    getESC_Thro() + " " );
        }
        else if (receivedata.mid(28,2) == "0c") {
            //carmotor
            ReadESC_Speed = (receivedata.mid(4,4).toInt(&ok, 16));
//            if(mag_user1 >= 32768){
//                mag_user1 -= 65536;
//            }
            Read_ESCHall = (receivedata.mid(8,4).toInt(&ok, 16));
//=============ace for esc
            addserialSaveAndApp("Carmotor_Data: " +
                                    getESC_Speed() + " " +
                                    getESC_Hall() + " " );
...
//========================================================fcg for step
QString SerialTest::getStepmotor_Angle(void) {
	return QString::number(StepMotorAngle, 'f', 0);
}//fcg
QString SerialTest::getStepmotor_Speed(void){
    return QString::number(StepMotorSpeed, 'f', 0);//fcg
}
//========================================================fcg for BDC
QString SerialTest::getBDCmotor_Angle(void){
    return QString::number(BDCMotorAngle, 'f', 0);//fcg
}
QString SerialTest::getBDCmotor_Speed(void){
    return QString::number(BDCMotorSpeed, 'f', 0);//fcg
}
//========================================================ace for ESC
QString SerialTest::getESC_Thro(void){
    return QString::number(ReadESC_Thro, 'f', 4);//
}
QString SerialTest::getESC_Speed(void){
    return QString::number(ReadESC_Speed, 'f', 2);//
}
QString SerialTest::getESC_Hall(void){
        return QString::number(Read_ESCHall, 'f', 4);//
    }
//========================================================ace for ESC
