...
//MTCNN::MTCNN(string proto_model_dir) {
//MTCNN::MTCNN() 
//{
	...
	/*PNet_ = cv::dnn::readNetFromCaffe("model/det1_.prototxt", "model/det1_.caffemodel");
	RNet_ = cv::dnn::readNetFromCaffe("model/det2.prototxt", "model/det2_half.caffemodel");
...
cv::Mat MTCNN::getwarpAffineImg(cv::Mat src, float landmarks[])
{
	cv::Mat oral;
	src.copyTo(oral);

	cv::Point2f eyesCenter = cv::Point2f((landmarks[0] + landmarks[2]) * 0.5f, (landmarks[1] + landmarks[3]) * 0.5f);


	double dy_l = (landmarks[3] - landmarks[1]);
	double dx_l = (landmarks[2] - landmarks[0]);
	double angle_l = atan2(dy_l, dx_l) * 180.0 / CV_PI; // Convert from radians to degrees.


	cv::Mat rot_mat_l = getRotationMatrix2D(eyesCenter, angle_l, 1.0);

	cv::Mat rot;
	//cv::Mat rot(112, 112, CV_8UC3);

	cv::warpAffine(src, rot, rot_mat_l, src.size());

	vector<cv::Point2f> marks_l;
	vector<cv::Point2f> marks_c;
	//int length = sizeof(landmarks) / sizeof(landmarks[0]);
	//std::cout << length << std::endl;
	for (int n = 0; n < 5; n++)
	{
		cv::Point2f pl = cv::Point2f(0, 0);
		pl.x = rot_mat_l.ptr<double>(0)[0] * landmarks[n * 2] + rot_mat_l.ptr<double>(0)[1] * landmarks[n * 2 + 1] + rot_mat_l.ptr<double>(0)[2];
		pl.y = rot_mat_l.ptr<double>(1)[0] * landmarks[n * 2] + rot_mat_l.ptr<double>(1)[1] * landmarks[n * 2 + 1] + rot_mat_l.ptr<double>(1)[2];
		//std::cout <<  "( " << pl.x << ", " << pl.y << " )" << std::endl;
		marks_l.push_back(pl);
	}
	int left = 1.625 * marks_l[0].x - 0.625 * marks_l[1].x;
	int top = marks_l[0].y - marks_l[3].y + marks_l[2].y;
	int right = 1.625 * marks_l[1].x - 0.625 * marks_l[0].x;
	int bottom = marks_l[3].y + marks_l[3].y - marks_l[2].y;
	int width = right - left + 1;
	int height = bottom - top + 1;

	//cv::rectangle(rot, cv::Rect(left, top, width, height), cv::Scalar(0, 255, 0), 2);


	//for (int j = 0; j < 5; j++)
	//{
	//	cv::circle(rot, marks_l[j], 1, cv::Scalar(255, 0, 0), 5);
	//}

	cv::Mat wraprot;
	if (left > 0 && top > 0 && right < rot.cols && bottom < rot.rows)
	{
		wraprot = rot(cv::Rect(left, top, width, height));
	}
	else
	{
		wraprot = NULL;
